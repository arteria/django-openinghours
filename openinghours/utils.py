...
from openinghours.app_settings import TIMEZONE_FIELD
...
def get_location_timezone(location):
    if not isinstance(TIMEZONE_FIELD, str):
        raise ImproperlyConfigured(
            "OPENINGHOURS_PREMISES_TIMEZONE_FIELD must be a string. ie, 'timezone'"
        )
...
def is_open(location, now=None):
    ...
    for oh in ohs:
        ...
        if (
            ...
            and oh.from_hour >= now_time
            and oh.to_hour >= now_time
            and oh.to_hour < oh.from_hour
        ):
            is_open = oh
